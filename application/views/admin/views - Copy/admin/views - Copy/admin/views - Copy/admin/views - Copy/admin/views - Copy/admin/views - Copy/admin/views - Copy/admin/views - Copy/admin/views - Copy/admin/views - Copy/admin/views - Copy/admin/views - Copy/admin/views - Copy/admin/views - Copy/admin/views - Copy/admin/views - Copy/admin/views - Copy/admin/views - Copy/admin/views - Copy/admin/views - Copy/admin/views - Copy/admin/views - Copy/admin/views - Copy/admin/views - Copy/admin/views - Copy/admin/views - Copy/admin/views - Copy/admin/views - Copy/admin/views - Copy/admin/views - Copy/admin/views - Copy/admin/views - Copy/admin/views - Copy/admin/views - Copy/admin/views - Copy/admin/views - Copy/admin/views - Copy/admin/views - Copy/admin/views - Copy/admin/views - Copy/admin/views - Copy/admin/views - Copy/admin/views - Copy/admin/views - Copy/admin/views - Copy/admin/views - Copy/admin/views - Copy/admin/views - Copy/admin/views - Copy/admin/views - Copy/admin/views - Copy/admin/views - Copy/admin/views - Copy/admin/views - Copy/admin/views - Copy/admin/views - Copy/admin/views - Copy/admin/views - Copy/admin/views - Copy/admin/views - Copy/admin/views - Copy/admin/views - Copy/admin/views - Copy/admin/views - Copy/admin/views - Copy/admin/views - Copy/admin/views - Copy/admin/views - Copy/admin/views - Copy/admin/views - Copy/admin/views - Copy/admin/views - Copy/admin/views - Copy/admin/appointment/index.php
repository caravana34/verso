                                    ...
                                    <table class="table table-striped table-bordered table-hover ajaxlist" data-export-title="<?php echo $this->lang->line('appointment_details'); ?>" >
                                          <thead>
                                            <tr>
                                              ...
                                              <th width="7%">Documento Identidad</th>
                                              <th width="18%">Motivo de consulta</th>
           ...
           <form id="rescheduleform" accept-charset="utf-8" method="post">
             <div class="row d-flex flex-wrap">
                ...
                <input type="hidden" name="appointment_id" id="appointment_id">
                 ...
                 <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                      <label>Motivo de consulta</label><small class="req"> *</small>
                      <div class="p-2 select2-full-width">
                  ...
                  <div class="col-sm-6" hidden>
                    ...
                    <div class="col-sm-3">
                      <div class="input-group">
                        ...
                        <small class="req"> *</small>
                        <div>
                          <select name="rdoctor" class="form-control" onchange="getDoctorFeesEdit(this);get_specialist(this,'edit')" style="width:100%" id="rdoctor" >
                            <option value="<?php echo set_value('doctor'); ?>"><?php echo $this->lang->line('select') ?></option>
                            <?php foreach ($doctors as $dkey => $dvalue) {
                            ?>
                 ...
                 <div id="moderadora" class="col-sm-3" style="display:none;">
                  ...
                  <div id="payment_type" class="col-lg-6" style="display: none; margin-top: 10px;">
                    <div>
                  ...
                  <div id="content_cancel" class="col-sm-3" style="display: none;">
                   ...
                   <div  class="col-sm-3" style="">
                    ...
                    <div class="col-sm-12">
                      <div class="form-group">
                        <label for="edit_medical_message">Mensaje para el medico</label>                 
...
//               console.log(vigenciadays);
            ...
            if(!tippy_content && holiday && blockeddays && vigenciadays && start_date >= today && doctor_id.value != "" && does_not_work && user_role_id != "3"){
...
//               var tiempo_final = hora.setMinutes(hora.getMinutes() + 10);
       ...
       slotLaneDidMount: function(info) {
          console.log(info);
//           var slotElement = info.el;
//           slotElement.style.border = '1px solid red';
       }, 
       ...
       dayCellContent: function (arg) {
           ...
           if(doctor_id !== '' && arg.view.type === 'timeGridDay' && businessHours){
                  ...
                  let filteredBusinessHours = businessHours.filter(function (interval) {
                   ...
                   fetch('<?= base_url("admin/appointment/update_block_days/") ?>'+doctor_id)
                   ...
                   fetch('<?= base_url("admin/appointment/vigencia/") ?>'+doctor_id)
                    .then(response => response.json())
                    .then(data => {
                     console.log(data);

                     if (data[0].end_date < events_date) {
                    ...
                    fetch('<?= base_url("admin/appointment/get_doctor/") ?>', {
                          method: "POST",
                          headers: {
                    ...
                    .then(data =>{ 
                           ...
                           filteredBusinessHours.forEach(function (interval) {
                                fc_timegrid_slot.forEach((element) => {  
                                    ...
                                    if(dataTimeAttributeValue >= interval.startTime && dataTimeAttributeValue < interval.endTime && !holidays.includes(this_date)){
                                        element.classList.add('available-time');
                                      
                                        if(user_role_id == 3){          
                                            element.innerHTML = `<h6 style="margin-left: 5px; color: white; font-size: 2rem;">${data.name} - ${data.specialist_doc}</h6>`;
                                        }else{
                                            element.innerHTML = `<h6 style="margin-left: 5px; color: white; font-size: 2rem;">${data.name} - ${data.specialist_doc}</h6>`;
                                        }
                                    }
          ...
          if (info.view.type === 'dayGridMonth') {
              ...
              return {
                html: `<div class="d-flex justify-content-between" style="margin-left: 5px; flex-wrap: wrap;">
                           <div style="display: flex; flex-wrap: wrap;">
                              <h4 style="margin: 0px;"><strong>Tiempo:</strong> ${info.event.extendedProps.data.time} - ${info.event.extendedProps.data.time_finish} </h4>&nbsp;
                              <h4 style="margin: 0px;"><strong>Doctor:</strong> ${info.event.extendedProps.data.name} ${info.event.extendedProps.data.surname}</h4>&nbsp;&nbsp;
                              <h4 style="margin: 0px;"><strong>Estado:</strong> <small class="label ${info.event.extendedProps.status_color}" style="border-color: 1px solid white;">${info.event.extendedProps.data.appointment_status}</small></h4>
                            </div>
                            <div>
                                ${info.event.extendedProps.options_html}
                            </div>
...
//                         window.open("https://clinify.co/cliniverso_dev/admin/bill/opd_visit_detail/4642/96", "PopupWindow", "width=600,height=400");
                             $.ajax({
                                url: "<?= base_url("admin/charges/charges_clini") ?>",
                                type: 'POST',
                                data: {data:data},
                                success:(resp)=> {
                                  resp = JSON.parse(resp);
                                  console.log(resp);
                                  window.location.assign("<?= base_url('admin/bill/opd_visit_detail/') ?>"+data.patient_id+"/"+data.opd_details_id+"#charges");
//                                   window.open("<?= base_url('admin/bill/opd_visit_detail/') ?>"+data.patient_id+"/"+data.opd_details_id+"#charges", "PopupWindow", "width=" + screen.width + ",height=" + screen.height);
//                                   window.open("https://clinify.co/cliniverso_dev/admin/bill/opd_visit_detail/"+data.patient_id+"/"+data.opd_details_id+"#charges", "PopupWindow", "width=" + screen.width + ",height=" + screen.height);
                                },
                                error: function() {
                                     console.error("No es posible completar la operación");
                                }
...
function printAppointment(id){
  ...
  //           $("#rslot_edit").val(data.shift_id);
            ...
            $("#edit_responsible_eps").val(data.responsible).trigger("change");
...
//             let global_shift = $("#rglobal_shift_edit").val();
   ...
   function setSlot(id){
        ...
        if($("#slot_"+id).data("filled") === "filled"){
            errorMsg("<?php echo $this->lang->line('not_available'); ?>");
        }else{
