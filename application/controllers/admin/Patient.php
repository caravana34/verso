...
//         $this->form_validation->set_rules('amount', $this->lang->line('amount'), 'trim|xss_clean|valid_amount');
//         $this->form_validation->set_rules('paid_amount', $this->lang->line('paid_amount'), 'trim|xss_clean|valid_amount');
//         $this->form_validation->set_rules('charge_id', $this->lang->line('charge'), 'trim|xss_clean');
        $this->form_validation->set_rules('appointment_date', $this->lang->line('appointment_date'), 'trim|xss_clean');
        $this->form_validation->set_rules('consultant_doctor', $this->lang->line('consultant_doctor'), 'trim|xss_clean');
//         $this->form_validation->set_rules('document', $this->lang->line('document'), 'callback_handle_doc_upload[document]');
        if ($this->form_validation->run() == false) {
            $msg = array(
                'charge_id'         => form_error('charge_id'),
            ...
            $email             = $this->input->post('email');
            $mobileno          = $this->input->post('mobileno');
            $opdn_id           = $this->input->post('opd_id');
            $custom_field_post = $this->input->post("custom_fields[opd]");
            $appointment_date  = $this->input->post('appointment_date');
...
//             }
            ...
            if (!empty($custom_value_array)) {
                $this->customfield_model->insertRecord($custom_value_array, $opdn_id);
                $array = array('status' => 'success', 'error' => '', 'id' => $opdn_id, 'patient_id' => $patient_id, 'message' => $this->lang->line('success_message'));
                echo json_encode($array);
                exit;
            }
//               echo "<pre>";
//        print_r($opd_visit_data);
//        exit;
...
//                     exit;
            ...
            if (isset($_FILES["custom_fields"]) && !empty($_FILES['custom_fields']['name'])) {
                foreach($_FILES["custom_fields"]['name']['patient'] as $key=>$value){
                    ...
                    if($value != ""){
                            $custom_fileInfo = pathinfo($value);
                            $custom_img_name = $alfa_no . "_" . $insert_id.'_'. $key . '.' . $custom_fileInfo['extension'];
                            move_uploaded_file($_FILES["custom_fields"]["tmp_name"]['patient'][$key], "./uploads/patient_images/attach/" . $custom_img_name);
                            $custom_data_img = array('id' => $insert_id,'custom_field_id'=>$key, 'image' => 'uploads/patient_images/attach/' . $custom_img_name);

                            $this->patient_model->add_custom($custom_data_img);
                        }
                }
            }
 ...
 */
    ...
    public function search()
    {
        ...
        //desarrollo clini$this->db->insert("visit_details", $opd_data);
        $sql="SELECT patients.*, opd_details.id AS id_case, COUNT(patients.id) as count,opd_details.case_reference_id, visit_details.appointment_date
                FROM patients
                INNER JOIN opd_details ON patients.id = opd_details.patient_id
                INNER JOIN visit_details ON opd_details.case_reference_id = visit_details.opd_details_id
                GROUP BY patients.id";
        $query = $this->db->query($sql);
        $result2 = $query->result_object();
//         $this->db->select('*');
    ...
    { 
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
                ...
                $action .= "<a href=" . base_url() . 'admin/patient/profile/' . $value->pid . " style=color:#000; class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";
                $action .= "</div'>";
                $first_action = "<a style='color:#1444BD;font-weight: bold;' href=" . base_url() . 'admin/patient/profile/' . $value->pid . ">";
                //==============================
                $row[] = $first_action . composePatientName($value->patient_name." ".$value->guardian_name, $value->pid). "</a>" . $action;
    ...
    public function getopdvisitdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
              ...
              $this->db->where('belong_table_id', $patientid);
              $query2 = $this->db->get();
...
// desarrollo cliniverso
    public function visitdetails($id, $opdid)
    {
        ...
        if(!empty($opdid)){
            if (!empty($id)) {
              ...
              $data['fields']             = $this->customfield_model->get_custom_fields($id, $opdid);
              $doctorid                   = "";
              $doctor_restriction         = $this->session->userdata['hospitaladmin']['doctor_restriction'];
              $disable_option             = false;
              if ($doctor_restriction == 'enabled') {
  ...
  //             $data['custom_fields_value'] = display_custom_fields('patient', $id);
...
//             exit;
    ...
    public function saveInability($id_user, $opdid){
      ...
      $basic = array(
          ...
          'inability_initial_date' => $this->input->post('inability_initial_date'),
          'inability_final_date' => $this->input->post('inability_final_date'),
          'inability_duration' => $this->input->post('inability_duration'),
          'inability_type_disability' => $this->input->post('inability_type_disability'),
          'inability_classification' => $this->input->post('inability_classification'),
          'inability_diagnosis' => $this->input->post('inability_diagnosis'),
          'inability_observation' => $this->input->post('inability_observation'),
      );
      $this->db->insert("inability", $basic);
      redirect('admin/patient/visitdetails/'. $id_user . '/' . $opdid . '#timeline');
    }
  ...
  public function visitdetailsTest()
    {
            $this->load->view("layout/header");
            $this->load->view("admin/patient/copyProfile");
            $this->load->view("layout/footer");
    }
  
  
      // Nuevo metodo para insertar los medicamentos a la base de datos
     ...
     public function insert_medication($user_id, $visit_id, $opdid){
       ...
       $post= $this->input->post();
        ...
        if ($this->form_validation->run() == FALSE) {
             ...
             $this->session->set_flashdata('mensaje', 'Mensaje de ejemplo');
             redirect('admin/patient/visitdetails/'. $user_id . '/' . $opdid . '#labinvestigation');
...
//        exit;
    ...
    //          'pharmacy_id' => $this->input->post('medication_concentration');
    //          'dosage' => $this->input->post('pharmaceutical_form');
    //          'dose_interval_id' => $this->input->post('medication_through');
    //          'dose_duration_id' => $this->input->post('medication_dose');
    //          'instructions' => $this->input->post('medication_periodicity');
    //          'created_at' => $this->input->post('medication_day');

          $this->prescription_model->insertPrescriptionDetails($details);

          print_r('se registro con exito');
    ...
    public function profile($id, $appoint_id_visit=null)
    {
        if (!$this->rbac->hasPrivilege('opd_patient', 'can_view')) {
            access_denied();
        }
        
        if(isset($appoint_id_visit)){
            $data['appoint_id_visit'] = $appoint_id_visit;
        }
        ...
        $data['medicines'] = $this->db->query("SELECT ipd_prescription_basic.id, ipd_prescription_details.id, ipd_prescription_basic.visit_details_id, ipd_prescription_basic.header_note, ipd_prescription_basic.footer_note, ipd_prescription_basic.finding_description, ipd_prescription_basic.is_finding_print, ipd_prescription_basic.date, ipd_prescription_basic.generated_by, prescribe_by,
                                                 ...
                                                 WHERE patients.id = ".$id.";")->result();
      
       $data['procedures'] = $this->db->query("SELECT procedures_clini.*
                                               FROM procedures_clini    
...
//         print_r($result);
//         exit;
        $result['patients_name']      = $result['patient_name']." ".$result['guardian_name'];
...
//         }
        ...
        foreach($result['field_data'] as $s=>$y){
            if($y->custom_field_id==12){
                $result['eps']  = $y->field_value;
            } 
            if($y->custom_field_id==10){
                $result['regimen']  = $y->field_value;
            }
            if($y->custom_field_id==28){
...
//         $result['einsurance_validity'] = $this->customlib->YYYYMMDDTodateFormat($result['insurance_validity']);
        $result['einsurance_validity'] = $result['insurance_validity'];
...
//                 'insurance_validity'    => $this->customlib->dateFormatToYYYYMMDD($this->input->post('validity')),
                'insurance_validity'    => $this->input->post('validity'),
                'known_allergies'       => $this->input->post('known_allergies'),
            );
            ...
            if (isset($_FILES["custom_fields"]) && !empty($_FILES['custom_fields']['name'])) {
                foreach($_FILES["custom_fields"]['name']['patient'] as $key=>$value){
                    if($value != ""){
                        $custom_fileInfo = pathinfo($value);
                        $custom_img_name = $alfa_no . "_" . $id .'_'. $key . '.' . $custom_fileInfo['extension'];
                        move_uploaded_file($_FILES["custom_fields"]["tmp_name"]['patient'][$key], "./uploads/patient_images/attach/" . $custom_img_name);
                        $custom_data_img = array('id' => $id,'custom_field_id'=>$key, 'image' => 'uploads/patient_images/attach/' . $custom_img_name);
...
//         exit();
    ...
    public function opd_detail_update()
    {
      $post = $this->input->post();
        ...
        $this->form_validation->set_rules('amount',$this->lang->line('amount'),'trim|xss_clean|valid_amount');
        $this->form_validation->set_rules('payment_date', $this->lang->line('payment_date'), 'trim|xss_clean');
        $this->form_validation->set_rules('payment_mode', $this->lang->line('payment_mode'), 'trim|xss_clean');
...
//           redirect('admin/patient/visitdetails/'. $user_id . '/' . $opdid . '#labinvestigation');
             ...
             $array = array('status' => 'success', 'error' => '', 'message' => 'El medicamento se registro con exito.');
        ...
        if (isset($total_rows) && !empty($total_rows)) {
            foreach ($total_rows as $row_key => $row_value) {
                ...
                if ($medicine_category !== "") {
                    ...
                    if ($dosage == "") {
                        $this->form_validation->set_rules('dosage', $this->lang->line('dosage'), '');
                    }
        ...
        if (isset($search_term) && $search_term != '') {
            ...
            if (!empty($result)) {
                ...
                foreach ($result as $value) {
                    $data[] = array("id" => $value->id, "text" => $value->patient_name." ".$value->guardian_name . " ( C.C : " . $value->identification_number . ")");
                }
            }
            echo json_encode($data);
        }
    ...
    public function editpayment()
    {
      ...
      public function insert_procedure($user_id, $visit_id, $opdid){
...
//       exit;
      ...
      if($json_data['atc']=="J05AR01") {
            $array = array('status' => "success", 'error' => '', 'message' => "No tomar durante la lactancia");
            echo json_encode($array);
            exit;
        }
      if($json_data['descripcionatc']=="ENALAPRIL") {
            $array = array('status' => "success", 'error' => '', 'message' => "No tomar durante la lactancia");
            echo json_encode($array);
            exit;
        }
...
//       if($json_data['descripcionatc'] === "PARACETAMOL" || $json_data['descripcionatc'] === "IBUPROFENO"
//         || $json_data['descripcionatc'] === "KETOPROFENO" || $json_data['descripcionatc'] === "NAPROXENO" 
//         || $json_data['descripcionatc'] === "DICLOFENACO" || $json_data['descripcionatc'] === "PIROXICAM"){
        
//         $array = array('status'=> "success",'message'=> "Analgesicos de venta libre",'error'=> '');
...
//       }   
      ...
      if (in_array($json_data['descripcionatc'], $analgesicos)) {
        $array = array('status' => "success", 'error' => '', 'message' => "Analgesicos de venta libre");
        echo json_encode($array);
        exit;
      }
...
//           ""
//         ),
//         // Agrega más categorías con sus palabras clave correspondientes aquí
//       );

//       foreach ($keywords as $category => $categoryKeywords) {
//         if (in_array($json_data['descripcionatc'], $categoryKeywords)) {
//           $array = array('status' => "success", 'error' => '', 'Category': "", 'message' => "");
//           echo json_encode($array);
//           exit;
//         }
//       }
...
//       exit;
   ...
   public function api_hl7_standard($opd_id,$id){
     ...
     if($result['result']['is_dead'] === "no"){
       ...
       $death_date = $result['result']['discharge_date'];
     }
     foreach($custom_fields_data  as $key=>$value){
          
          switch ($value->custom_field_id) {
            case 3:
                $country = $value->field_value;
                break;
            case 5:
                $city = $value->field_value;
                break;
            case 7:
                $contact_person = $value->field_value;
                break;
            case 11:
              $phone_contact_person = $value->field_value;
              break;
            case 25:
              $address = $value->field_value;
            break;
            case 30:
              $contact = $value->field_value;
            break;
...
//       }
  ...
  public function finish_attention(){
    echo '<pre>';
    print_r('entro aqui');
    exit;
  }
