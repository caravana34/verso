...
class Admin extends Admin_Controller
{
    ...
    public function search()
    {
       if (!$this->rbac->hasPrivilege('patient', 'can_view')) {
        ...
        $this->session->set_userdata('top_menu', 'patients');
    ...
    public function getpatientdatatable()
    {
        ...
        if (!empty($dt_response->data)) {
            foreach ($dt_response->data as $key => $value) {
              ...
              $this->db->where('belong_table_id', $value->id);
              $query2 = $this->db->get();
...
//                     $action .= "<ul class='dropdown-menu dropdown-menu2' role='menu'>";
                    ...
                    foreach ($result[$key]['info'] as $pkey => $pvalue) {
                        $action .= "<li>" . "<a href='" . $result[$key]['url'][$pkey] . "' class='btn btn-default btn-xl'  data-toggle='' title=''>" . $pvalue . "</a>" . "</li>";
                    }
